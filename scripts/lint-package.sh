...
lint_one() {
  ...
  # External deps are version-tracked only — not built here, skip all checks.
  local external
  external=$(yq e ".${key}.external // false" versions.yml 2>/dev/null || echo false)
  if [[ "$external" == "true" ]]; then return; fi
